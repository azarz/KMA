...
if isfield(options,'printing') == 0
    options.printing = 0;   % default: no printing
end
...
B = Ld; %+lambda*eye(size(Ld));         % (n1+n2) x (n1+n2) 
...
switch lower(options.kernelt)
    ...
    case {'rbf','chi2','histlag'}
        ...
        for i = 1:options.numDomains
            eval(sprintf('K%i = kernelmatrix(''%s'',X%i,X%i,options.sigma{%i});',i,options.kernelt,i,i,i));
            eval(sprintf('K = blkdiag(K,K%i);',i));
        end
...
[ALPHA,LAMBDA,options.projections] = gen_eig(KAK,KBK,'LM'); % options.projections
...
if strcmpi(options.kernelt,'wan')
    for i = 1:options.numDomains
