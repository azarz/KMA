function [proj, d] = KMAproject(labeled,unlabeled,test,ALPHA,options)
...
switch lower(options.kernelt)      
    ...
    case {'rbf','chi2','histlag'}
        for i = 1:options.numDomains
            eval(sprintf('K%i = kernelmatrix(''%s'',X%i,X%i,options.sigma{1,%i});',i,options.kernelt,i,i,i));
%             disp('tololtotloo');disp(size(X1));disp(size(XT1)); disp('tololtotloo');
            eval(sprintf('KT%i = kernelmatrix(''%s'',X%i,XT%i,options.sigma{1,%i});',i,options.kernelt,i,i,i));
        end
...
for i = 1:options.numDomains
    ...
    li = size(PhiTtoF,2);
    eval(sprintf('PhiTtoF = ((PhiTtoF'' - repmat(m,%i,1))./ repmat(s,%i,1))'';',li,li));
